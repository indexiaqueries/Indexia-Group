    ...
    <Reveal>
      <article className="group relative grid overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm transition-shadow duration-300 hover:shadow-lg lg:grid-cols-[42%_1fr]">
        ...
        <div className="relative overflow-hidden">
          <ImageSlot
            {...siteImages.newsFeatured}
            alt={featured.title}
            aspect="aspect-[4/3] lg:aspect-auto lg:h-full"
            className="rounded-none border-0 object-cover transition-transform duration-500 group-hover:scale-[1.03]"
          />
        </div>
          ...
          <div>
            ...
            <div className="relative mt-6 flex flex-wrap items-center gap-4">
              {companies.find((c) => c.name === featured.company) && (
                <Link
                  to={`/${companies.find((c) => c.name === featured.company)?.slug ?? ""}`}
